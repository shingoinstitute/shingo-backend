...
// router.route('/:id')
//   .get(function(req, res,next) {
//     var filename = 'workshops_' + req.params.id;
//     var force_refresh = req.query.force_refresh ? req.query.force_refresh : false;
//     if (cache.needsUpdated(filename, 30) || force_refresh) {
//       var query = "SELECT Id, Name, Host_Site__c, Start_Date__c, End_Date__c, Event_City__c, Event_Country__c, Organizing_Affiliate__r.Name, Organizing_Affiliate__r.Page_Path__c, Workshop_Type__c, Registration_Website__c FROM Workshop__c WHERE Public__c=true AND Status__c='Verified' AND WHERE Id='" + req.params.id + "'";
//       SF.queryAsync(query)
//         .then(function(results) {
//           var response = {
//             success: true,
//             workshops: results.records[0]
//           }
//
//           res.json(response);
//           return cache.addAsync(filename, response);
